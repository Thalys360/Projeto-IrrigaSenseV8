...
async function saveZoneConfig(zone, config) {
  requireAdmin();
  await update(ref(db, `irrigasense/zonas/zona${zone}/config`), {
    ...config,
    atualizadoEm: serverTimestamp()
  });
}

async function saveIrrigationSchedule(zone, schedule) {
  requireAdmin();
  const zoneNumber = Number(zone);
  if (![1, 2].includes(zoneNumber)) throw new Error("INVALID_ZONE");
  if (!schedule || typeof schedule !== "object") throw new Error("INVALID_SCHEDULE");

  await set(ref(db, `irrigasense/zonas/zona${zoneNumber}/config/programacao`), {
    ...schedule,
    atualizadoEm: serverTimestamp()
  });
}
...
import("./irrigation-schedule.js?v=20260820a").catch(error => {
  console.error("Falha ao carregar Irrigação Programada:", error);
});

import("./manual-countdown.js?v=20260820c").catch(error => {
  console.error("Falha ao carregar contagem regressiva manual:", error);
});


import("./visual-upgrade.js?v=20260820c").catch(error => {
  console.error("Falha ao carregar visual do mascote/culturas:", error);
});
