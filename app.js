...
function loadState(){
  try{
    const raw=localStorage.getItem("irrigasense2_site_state");
    if(!raw)return clone(DEFAULT_STATE);
    const s=JSON.parse(raw);
    return {
      ...clone(DEFAULT_STATE),
      ...s,
      zones:s.zones||clone(DEFAULT_STATE.zones),
      history:s.history||clone(DEFAULT_STATE.history),
      profiles:s.profiles||clone(DEFAULT_STATE.profiles),
      calibration:s.calibration||clone(DEFAULT_STATE.calibration),
      security:{},
      flowRates:{...clone(DEFAULT_STATE.flowRates),...(s.flowRates||{})},
      waterUsage:{...clone(DEFAULT_STATE.waterUsage),...(s.waterUsage||{})},
      // Mantemos imagens antigas somente em memória para conseguir recuperá-las.
      // Novas fotos ficam exclusivamente no Firebase e não ocupam o localStorage.
      teamPhotos:{...clone(DEFAULT_STATE.teamPhotos),...(s.teamPhotos||{})},
      gallery:Array.isArray(s.gallery)?s.gallery:[]
    };
  }catch(e){
    console.warn("Estado local inválido; usando padrão.",e);
    return clone(DEFAULT_STATE);
  }
}
let state=loadState();
function saveState(){
  // Base64 de fotos pode ultrapassar rapidamente o limite (~5 MB) do localStorage.
  // Guardamos localmente apenas dados leves; galeria e fotos da equipe vivem no Firebase.
  const {gallery,teamPhotos,...lightState}=state;
  try{
    localStorage.setItem("irrigasense2_site_state",JSON.stringify(lightState));
  }catch(e){
    console.warn("Não foi possível salvar o estado local leve.",e);
  }
}
...
let galleryRecoveryNoticeShown=false;
function applyRemoteGallery(value){
  const remote=normalizeRemoteGallery(value);
  const local=normalizeRemoteGallery(state.gallery);

  // Migração da versão antiga: preserva fotos que ainda existam apenas no
  // navegador em vez de apagá-las quando o Firebase sincronizar.
  const remoteIds=new Set(remote.map(item=>String(item.id)));
  const recovered=local.filter(item=>!remoteIds.has(String(item.id)));
  state.gallery=[...remote,...recovered];

  saveState();
  renderGallery();
  if(currentAdminUser)renderAdminGallery();

  if(recovered.length&&!galleryRecoveryNoticeShown){
    galleryRecoveryNoticeShown=true;
    setTimeout(()=>toast(`${recovered.length} foto(s) local(is) recuperada(s). Ao adicionar ou reorganizar a galeria, elas serão sincronizadas com o Firebase.`),300);
  }
}
...
function resizeImage(file,maxWidth=900,quality=.70,maxChars=320000){
...
document.getElementById("addGalleryPhoto").addEventListener("click",async()=>{
  ...
  try{
    const src=await resizeImage(file,900,.68,320000);
    const item={
      id:Date.now(),
      src,
      title,
      ...
      order: state.gallery.length ? Math.min(...state.gallery.map(x=>Number(x.order)||0))-1 : 0
    };
    await firebaseApi().saveGalleryItem(item,item.order);
...
async function persistGallery(message){
  ...
  try{
    state.gallery.forEach((item,index)=>item.order=index);
    await firebaseApi().saveGalleryOrder(state.gallery);
    if(message)toast(message);
  }catch(e){
    console.error("Salvar ordem da galeria:",e);
    toast("A ordem mudou apenas neste aparelho; tente novamente.");
  }
}
...
function renderAdminGallery(){
  ...
  list.querySelectorAll("[data-delete-photo]").forEach(b=>b.addEventListener("click",async()=>{
    const id=Number(b.dataset.deletePhoto);
    if(!currentAdminUser)return toast("Entre como administrador.");
    try{
      await firebaseApi().removeGalleryItem(id);
      ...
      console.error("Excluir foto:",e);
      toast("Não foi possível excluir a foto do Firebase.");
    }
  }));
