...
function renderAdminGallery(){const l=document.getElementById("adminGalleryList");if(!l)return;if(!state.gallery.length){l.innerHTML='<div class="notice info">Nenhuma foto cadastrada.</div>';return}l.innerHTML=state.gallery.map(p=>`<article class="admin-gallery-item"><img src="${p.src}" alt=""><div><strong>${escapeHtml(p.title)}</strong><br><small>${escapeHtml(p.date||"Sem data")} • ${p.featured?"Destaque":"Galeria"}</small></div><button class="btn danger" data-delete-photo="${p.id}">Excluir</button></article>`).join("");l.querySelectorAll("[data-delete-photo]").forEach(b=>b.addEventListener("click",()=>{state.gallery=state.gallery.filter(x=>x.id!==Number(b.dataset.deletePhoto));saveState();renderGallery();renderAdminGallery();toast("Foto removida.")}))}
